...
class ReplayBuffer:
    ...
    *Note that we are indexing starting from 1*.
    ...
    def __init__(self, capacity, alpha, input_dims):
        """
        ### Initialize
        """
        ...
        # Arrays for buffer
        self.data = {
            'obs': np.zeros(shape=(capacity, input_dims), dtype=np.float32),
            'action': np.zeros(shape=capacity, dtype=np.int64),
            'reward': np.zeros(shape=capacity, dtype=np.float32),
            'next_obs': np.zeros(shape=(capacity, input_dims), dtype=np.float32),
